...
function buildFrame(state) {
  ...
  rows.push(paint(C.bold + C.cyan, "AI CLI 安装平台 — 从官方源一键安装终端编程工具"));
  rows.push("");
  tools.forEach((tool, i) => {
    ...
    const inst = st.installed
      ? paint(C.green, ` [已装${st.version ? ` ${st.version}` : ""}]`)
      : "";
    rows.push(
      ` ${cursor} ${mark} ${pad(tool.name, 22)}${pad(tool.vendor, 26)}最新 ${pad(latest, 14)}${inst}${paint(
        C.magenta,
        ` [${tool.tag}]`
      )}`
    );
  ...
  rows.push("");
  rows.push(
    paint(C.dim, "↑/↓ 选择 · Enter 安装 · v 验证 · r 刷新版本 · u 卸载(先备份) · i 详情 · q 退出")
  );
  if (busy) rows.push(paint(C.yellow, "⏳ 正在执行，请稍候..."));
  if (pendingConfirm) {
    const name = pendingConfirm.tool?.name ?? "";
    rows.push(paint(C.yellow, `确认卸载 ${name}？请按 w 确认，n 取消（Esc / Ctrl+C 退出）`));
    if (pendingConfirm.reminded) {
      rows.push(paint(C.yellow, "⚠️ 请按 w 或者 n（Esc / Ctrl+C 退出）"));
    }
  }
  return rows;
}
