...
const zh = {
  ...
  install_py_missing: "❌ {name} 需要 Python，但本机未检测到 Python。请先安装 Python 3.10–3.12（https://www.python.org/downloads/）后重试",
  install_py_unsupported: "❌ {name} 需要 Python {need}，当前是 Python {have}。{name} 上游尚未适配该 Python 版本，直接安装会因依赖编译失败而报错。\n   解决方案: {hint}",
  ...
  menu_option4: "一键启动（选 Key → 选模型 → 选工具 → 启动）",
  menu_footer: "↑/↓ 选择 · 数字直选 · Enter 确认 · q 退出",
  ...
  wiz_install_failed_abort: "⚠️ {name} 安装失败，已中止向导（未生成配置）。可到「下载 / 拉取工具」查看原因（安装 Python 3.10–3.12 后重试）",
  ...
  // 一键启动
  launch_step1: "① 选择 API Key（已配置的提供商）",
  launch_no_keys: "❌ 还没有配置任何 API Key，请先到「已配置概览 → API Key 管理」添加",
  launch_bad_model: "❌ 模型名含有非法字符（只允许字母/数字/.-_/:），已取消",
  launch_step2: "② 选择模型（{provider}）",
  ...
  launch_install_failed_abort: "⚠️ {tool} 安装失败，已中止启动。可到「下载 / 拉取工具」查看原因（安装 Python 3.10–3.12 后重试）",
  launch_blocked: "🚫 {tool} 无法用 {provider} 原生接入",
  launch_blocked_hint: "按 Enter 返回主菜单，换个支持该协议的厂商，或用 LiteLLM 网关做协议转换。",
  launch_gateway_up: "🛰️ 已自动启动内建协议网关，代理 {target} 到本地端口 {port}（退出后自动回收）",
  launch_gateway_failed: "❌ 内建网关启动失败: {msg}（{target}）",
  compat_needs_gateway: "{provider} 不提供 Codex 所需的 Responses 端点，已自动接入内建协议网关（Responses→Chat 转换）。网关端口为运行时动态分配，静态命令无法预知，请回到主菜单用「一键启动」直接拉起，无需手动配置。",
};
...
const en = {
  ...
  install_py_missing: "❌ {name} needs Python, but none was detected on this machine. Install Python 3.10–3.12 first (https://www.python.org/downloads/) and retry",
  install_py_unsupported: "❌ {name} requires Python {need}, but Python {have} is installed. {name} upstream has not adapted to that Python yet, so the install would fail on dependency builds.\n   Fix: {hint}",
  ...
  compat_command: "Run directly: {cmd}",
  compat_apply_hint: "PowerShell: Get-Content '{file}' | ForEach-Object {{ $env:{k} = $_ }}",
  ...
  menu_option4: "One-click launch (key → model → tool → start)",
  menu_footer: "↑/↓ select · number keys jump · Enter confirm · q quit",
  ...
  wiz_install_failed_abort: "⚠️ {name} install failed — wizard aborted (no config generated). Check the tool list for details (install Python 3.10–3.12 and retry)",
  ...
  // 一键启动
  launch_step1: "① Choose an API key (configured providers)",
  launch_no_keys: "❌ No API keys configured yet — add one in Settings & overview → API keys first",
  launch_bad_model: "❌ Model name contains illegal characters (only letters/digits/.-_/: allowed), cancelled",
  launch_step2: "② Choose a model ({provider})",
  ...
  launch_install_failed_abort: "⚠️ {tool} install failed — launch aborted. Check the tool list for details (install Python 3.10–3.12 and retry)",
  launch_blocked: "🚫 {tool} can't natively use {provider}",
  launch_blocked_hint: "Press Enter to return to the main menu, switch to a provider that supports this protocol, or use a LiteLLM gateway to translate protocols.",
  launch_gateway_up: "🛰️ Auto-started the built-in protocol gateway, proxying {target} to local port {port} (auto-reclaimed on exit)",
  launch_gateway_failed: "❌ Failed to start the built-in gateway: {msg} ({target})",
  compat_needs_gateway: "{provider} doesn't expose the Responses endpoint Codex needs; it's auto-wired through the built-in gateway (Responses→Chat). The gateway port is allocated at runtime, so a static command can't know it — just use One-Click Launch from the main menu.",
};
