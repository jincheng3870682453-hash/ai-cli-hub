 ...
 */
async function fetchToolVersion(tool, cache) {
  const hit = cache[tool.id];
  if (tool.kind === "npm") {
    const info = await fetchNpmInfo(tool.pkg);
    if (info) {
      const entry = {
        version: info.latest,
...
async function refreshAll(tools) {
  const cache = loadCache();
  const results = {};
  await Promise.all(
    tools.map(async (tool) => {
      results[tool.id] = await fetchToolVersion(tool, cache);
    })
  );
  saveCache(cache);
  return results;
